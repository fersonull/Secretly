...
export const CredentialsProvider = ({ children }) => {
  ...
  const addCredential = useCallback(async (credentialData) => {
    ...
    try {
      const result = await credentialsStorage.addCredential(user.id, credentialData);
      if (result.success) {
  ...
  // Update a credential
  const updateCredential = useCallback(async (credentialId, updates) => {
    if (!user?.id) {
      return { success: false, error: 'User not authenticated' };
    }
  ...
  const getCredentialById = useCallback((credentialId) => {
    return credentials.find(cred => cred.id === credentialId) || null;
  }, [credentials]);

  // Toggle favorite status
  const toggleFavorite = useCallback(async (credentialId) => {
    const credential = getCredentialById(credentialId);
    if (!credential) {
      return { success: false, error: 'Credential not found' };
    }

    return await updateCredential(credentialId, {
      isFavorite: !credential.isFavorite,
    });
  }, [getCredentialById, updateCredential]);
  ...
  const getCredentialsByCategory = useCallback((categoryId) => {
    if (categoryId === 'all' || !categoryId) {
      return credentials;
    }
    return credentials.filter(credential => credential.category === categoryId);
  }, [credentials]);
  ...
  const getRecentCredentials = useCallback((limit = 10) => {
    return [...credentials]
  ...
  const getWeakPasswordCredentials = useCallback(() => {
    return credentials.filter(credential => credential.passwordStrength === 'weak');
  }, [credentials]);
